...
const Header: FC = () => {
  ...
  return (
    <header>
      <div
        className="fixed top-0 left-0 w-full h-0 overflow-hidden bg-slate-900 z-10"
        ref={navScope}
      >
        <nav className="mt-20 flex flex-col">
          {navItems.map(({ label, href }) => (
            <a
              href={href}
              key={label}
              className="text-stone-200 border-t last:border-b-8 border-stone-800 py-8 group/nav-item relative isolate"
